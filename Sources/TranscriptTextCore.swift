...
struct TranscriptCommandParsingResult: Equatable {
...
enum TranscriptPasteTextFormatter {
    static func textToWrite(for transcript: String) -> String {
        guard let last = transcript.last, ".!?".contains(last) else { return transcript }
        return transcript + " "
    }
}
